...
const accountDatabase = new sqlite3.Database('./AccountDatabase.db', sqlite3.OPEN_READWRITE, (error: { message: any; }) => {
    if (error) {
...
ipcMain.handle('getRecentlyViewed', async (_event, accountId) => {
    return new Promise((resolve, reject) => {
        // Get the recently viewed products for the specific user
...
// Close the database connection when the application is about to quit
app.on('before-quit', () => {
    accountDatabase.close((error: { message: any; }) => {
        if (error) {
